...
class CellInterface {
public:
    // Текст ячейки, либо значение формулы, либо сообщение об ошибке из формулы
    using Value = std::variant<std::string, double, FormulaError>;

    virtual ~CellInterface() = default;

    /* Возвращает видимое значение ячейки.
     * В случае текстовой ячейки это её текст (без экранирующих символов).
     * В случае формулы - числовое значение формулы или сообщение об ошибке. */
     ...
     * В случае текстовой ячейки список пуст. */
...
public:
     ...
     * - Если задаётся синтаксически некорректная формула, то бросается исключение
     * FormulaException и значение ячейки не изменяется.
     * - Если задаётся формула, которая приводит к циклической зависимости (в
     * частности, если формула использует текущую ячейку), то бросается исключение
     * CircularDependencyException и значение ячейки не изменяется.
     * Уточнения по записи формулы:
     * - Если текст содержит только символ "=" и больше ничего, то он не считается формулой
     * - Если текст начинается с символа "'" (апостроф), то при выводе значения ячейки
     * методом GetValue() он опускается. Можно использовать, если нужно начать текст
     * со знака "=", но чтобы он не интерпретировался как формула. */
     ...
     * - Столбцы разделяются знаком табуляции. После каждой строки выводится символ перевода строки.
     * Для преобразования ячеек в строку используются методы GetValue() или GetText() соответственно.
